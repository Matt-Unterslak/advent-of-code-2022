...
numpy_grid = np.array(file_data)
print(f"{numpy_grid.shape=}")
    ...
    for col in range(1, numpy_grid.shape[1] - 1):
        ...
        for i in range(1, numpy_grid.shape[0] - 1):
            ...
            if up_visible is True:
                up = numpy_grid[row - i, col] if row - i >= 0 else numpy_grid[0, col]
                if tree <= up:
                    up_visible = False
                if row - i >= 0:
                    scenic_scores[0] += 1
            ...
            if left_visible is True:
                left = numpy_grid[row, col - i] if col - i >= 0 else numpy_grid[row, 0]
                if tree <= left:
                    left_visible = False
                if col - i >= 0:
                    scenic_scores[2] += 1
        ...
        if up_visible or down_visible or left_visible or right_visible:
            total_scenic_score = reduce(lambda x, y: x * y, scenic_scores)
